...
var obj = function () {
    ...
    this.create_plane = function () {

        //PlaneBufferGeometry = function ( width, height, widthSegments, heightSegments )
        this.geometry = new THREE.PlaneBufferGeometry(SCENE_WIDTH * 2, SCENE_HEIGHT * 2, this.mesh_detail, this.mesh_detail);
        this.geometry.applyMatrix(new THREE.Matrix4().makeRotationX(-Math.PI / 2));

        /*        for ( var i = 0, l = this.geometry.vertices.length; i < l; i ++ ) {

					this.geometry.vertices[ i ].y = Math.random() * 100;
...
this.geometry.vertices[ i ].y = 35 * Math.sin( i / 5 + ( time + i ) / 7 )
        ...
        for (var i = 0, j = 0, l = vertices.length; i < l; i++, j += 3) {
            vertices[j + 1] = 35 * Math.sin(i / 3 + (time * 2 + i) / 7);
        }
    ...
    this.update_perlin = function () {
        var speed = .03;
        perlin = new ImprovedNoise(), quality = this.noise_detail, this.z += Math.random() * speed;
        ...
        for (var i = 0, j = 0, l = vertices.length; i < l; i++, j += 3) {
            ...
            var x = i % SCENE_WIDTH,
                y = ~~ (j / SCENE_WIDTH);
            vertices[j + 1] = this.position.z + 120 / this.noise_detail * perlin.noise(x / quality, y / quality, this.z) * quality;
            //quality *= 5;
            //vertices[ j + 1 ] = 35 * Math.sin( i / 3 + ( time*2 + i ) / 7 );
        }
